...
function crypto() {
  ...
  instance = new Crypto(config.wechat.mp.Token, config.wechat.mp.EncodingAESKey, config.wechat.mp.AppID);
  return instance;
}
...
exports.verify = function (handler) {
  ...
  sha1.update([config.wechat.mp.Token, timestamp, nonce].sort().join(""));
