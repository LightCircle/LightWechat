...
var MPAPI = function api(appid, secret) {
  ...
  var api = new wapi(appid || config.wechat.mp.AppID, secret || config.wechat.mp.AppSecret);
  ...
  api.request = function (url, opts, callback) {
    ...
    request(parse(url, options), function (err, response, body) {
      if (err || response.statusCode != 200) {
        return callback(err || response.statusCode, response);
      }
...
function oauthUrl(redirect, option) {
  ...
  return encodeURIComponent("https://open.weixin.qq.com/connect/oauth2/authorize?"
    + "appid=" + (option.appid || config.wechat.mp.AppID) + "&"
...
function accessToken(option, callback) {
  ...
  var url = "https://api.weixin.qq.com/sns/oauth2/access_token?"
    + "appid=" + (option.appid || config.wechat.mp.AppID) + "&"
    + "secret=" + (option.secret || config.wechat.mp.AppSecret) + "&"
    + "code=" + option.code + "&"
...
function parse(url, option) {
  ...
  var result = {
    ...
    encoding: null // 接收binary data时,需要设定为null, 这样返回的data会是Buffer对象(不确定对普通请求的影响)
  };
